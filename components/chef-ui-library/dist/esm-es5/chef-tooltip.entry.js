...
import { P as root, G as toNumber, i as isObject, d as convert, r as require$$1 } from './chunk-6b826df5.js';
/**
 ...
 * </p>
 */
var ChefTooltip = /** @class */ (function () {
    function ChefTooltip(hostRef) {
    ...
    Object.defineProperty(ChefTooltip.prototype, "screenPosition", {
        get: function () {
            return this._screenPosition;
        },
        set: function (_a) {
            var x = _a[0], y = _a[1];
            var _b = this.offset.split(' ').map(function (i) { return parseInt(i, 10); }), offsetX = _b[0], offsetY = _b[1];
            this._screenPosition = [x + (offsetX || 0), y + (offsetY || 0)];
        },
        enumerable: true,
        configurable: true
    });
    ChefTooltip.prototype.componentDidLoad = function () {
        var ref = document.querySelector("#" + this.for);
        if (ref) {
            if (this.follow) {
    ...
    ChefTooltip.prototype.render = function () {
        var _a = this.screenPosition, x = _a[0], y = _a[1];
        var styles = { left: x + "px", top: y + "px" };
        var classNames = [
    ...
    ChefTooltip.prototype.setupDynamicPositioning = function (ref) {
        var _this = this;
        var delayedShow = debounce$1(this.delay, function () { return _this.visible = true; });
        var handleMouseEnter = function (event) {
            _this.screenPosition = [event.clientX, event.clientY];
        };
    ...
    ChefTooltip.prototype.setupStaticPositioning = function (ref) {
        var _this = this;
        var transitionTimeoutID, hideTimeoutID;
        var transitionDuration = parseFloat(getComputedStyle(this.el).transitionDuration) * 1000;
        var hideTooltip = function () {
            hideTimeoutID = setTimeout(function () { return _this.visible = false; }, _this.delay);
            transitionTimeoutID = setTimeout(handleTransitionComplete, transitionDuration);
        };
        var cancelHideTooltip = function () {
        ...
        var handleMouseMove = debounce$1(this.delay, function () {
            _this.setStaticPosition(ref);
            _this.visible = true;
        });
        var handleScroll = function () { return _this.setStaticPosition(ref); };
        var handleMouseLeave = function () {
            handleMouseMove.cancel();
            hideTooltip();
        };
        var handleTransitionComplete = function () { return removeEventListener('scroll', handleScroll); };
        var handleTooltipEnter = function () {
            cancelHideTooltip();
        };
        var handleTooltipLeave = function () {
            hideTooltip();
        };
    ...
    ChefTooltip.prototype.setStaticPosition = function (ref) {
        var boundingRect = ref.getBoundingClientRect();
        var top = boundingRect.top;
        var bottom = top + boundingRect.height;
        var left = boundingRect.left;
        var right = left + boundingRect.width;
        var centerXOffset = boundingRect.width / 2;
        var centerYOffset = boundingRect.height / 2;
